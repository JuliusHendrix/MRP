...
from src.neural_nets.NN_utils import multiple_MSELoss_dict, move_to
...
def model_step(device, model, example, loss_weights=None):
    ...
    mse_loss, loss_arr = multiple_MSELoss_dict(
        inputs=inputs,
        outputs=outputs,
...
def train_VAE(dataset_dir, save_model_dir, log_dir, params):
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, unit='batch', desc=f'Train epoch {epoch}') as train_epoch:
            ...
            for n_iter, example in enumerate(train_epoch):
                loss, loss_arr, kl_div = model_step(device, model, example, **params['loss_params'])
    ...
    metric_dict = {"Validation/KL": validation_kl_div}
...
def main():
    ...
    dataset_dir = os.path.join(MRP_dir, 'data/christmas_dataset/unscaled_dataset')
