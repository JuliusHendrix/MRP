...
from src.neural_nets.NN_utils import move_to, plot_core_y_mixs

from ae_params import ae_params


def encode_y_mixs(device, y_mixs, mrae_model):
    # mixing ratio's
    y_mixs_latent = torch.zeros(y_mixs.shape[0], mrae_model.latent_dim, y_mixs.shape[2]).double().to(device)  # [b, mrae_latent_dim, num_species]
    ...
    for key in models.keys():
        ...
        if state_dicts[key] is not None:
            model.load_state_dict(
                torch.load(os.path.join(save_model_dir, state_dicts[key]))
            )
        model.eval()
...
def encode_inputs_outputs(device, ae_models, example):
    # extract inputs
