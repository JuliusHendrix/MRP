...
def model_step(device, model, example, diff_weight, ddiff_weight, loss_weights):
    ...
    loss, loss_arr, diff_loss, ddiff_loss = loss_fn(device, inputs, outputs, diff_weight, ddiff_weight, loss_weights)
...
def train_autoencoder(dataset_dir, save_model_dir, log_dir, params):
    ...
    y_mix_ini_std = scaling_params['inputs']['y_mix_ini'][1]

    print(f'{y_mix_ini_std = }')
    ...
    if params['load_model']:
        print(f'loading state dict {params["state_dict"]}')
        model.load_state_dict(torch.load(os.path.join(save_model_dir, f'{params["state_dict"]}')))
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, unit='batch', desc=f'Train epoch {epoch}') as train_epoch:
            ...
            for n_iter, example in enumerate(train_epoch):
                loss, loss_arr, diff_loss, ddiff_loss = model_step(device, model, example,
                                                                   loss_weights=params['loss_params']['loss_weights'],
            ...
            for n_iter, example in enumerate(test_epoch):
                ...
                if n_iter % writer_interval == 0:
                    for i, el_loss in enumerate(loss_arr):
                        tot_ind_losses[i] += el_loss.detach()
...
def main():
    ...
    params = dict(
        name='AE2_large_d_conv_ks_5_std1.5_clipped',
