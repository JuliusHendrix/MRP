...
sys.path.append('../petitRADTRANS-master/')
...
def read_MUSCLES():
	...
	for i in range(len(NAMES)):	
		...
		spec_full = fits.getdata(file_full,1)
		flux_full = spec_full['FLUX']
		wavs_full = spec_full['WAVELENGTH']

		spec_HST = fits.getdata(file_HST,1)
...
def create_specs(save_to_txt = False):
	'''
	...
	colors = plt.cm.plasma(np.linspace(0,1,len(TEMP_grid)))
	...
	for i in range(len(TEMP_grid)):
		...
		if save_to_txt:
			...
			with open('output/txtfiles/{0}_K.txt'.format(T),'w+') as f:
				f.write(new_str)
			f.close()
		
	# 	im = ax.plot(wavs_total,flux_total,label='T = {0}'.format(T),color=colors[i])
	# ax.set_xlim(4,35000)
	# ax.set_xlabel('Wavelength',fontsize=12)
	# ax.set_ylabel('Flux',fontsize=12)
	# #plt.legend()
	# fig.colorbar(plt.cm.ScalarMappable(norm=(np.min(TEMP_grid),np.max(TEMP_grid)), cmap='plasma'))
	# plt.show()
	
	'''
		#plt.plot(wavs_total,flux_total)
		#plt.yscale('log')
		#plt.xscale('log')
		#plt.title('{0} K'.format(abs(Temps_MUS[Tdiff_indx] - T)))
		#plt.show()
		
	#plt.scatter(T_diff,spec_diff)
	#plt.ylabel('Spectra ratio')
	#plt.xlabel('T difference')
	#plt.show()
	'''
